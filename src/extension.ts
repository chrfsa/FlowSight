...
export function activate(context: vscode.ExtensionContext) {
    ...
    const batchHandler = (req: any, res: any) => {
        const body = req.body;
        if (body.post && Array.isArray(body.post)) body.post.forEach((r: any) => handleRun(r, 'start'));
        if (body.patch && Array.isArray(body.patch)) body.patch.forEach((r: any) => handleRun(r, 'end'));
        res.status(200).json({});
    };

    const createRunHandler = (req: any, res: any) => {
        handleRun(req.body, 'start');
    ...
    server = app.listen(1984, () => {
        console.log('🕵️ FlowSight UI écoute sur port 1984');
        vscode.window.showInformationMessage('FlowSight UI Ready (Port 1984)');
    });

    function handleRun(data: any, eventType: string) {
        if (currentPanel) currentPanel.webview.postMessage({ type: 'TRACE', data: data, event: eventType });
    }

    context.subscriptions.push(vscode.commands.registerCommand('flowsight.open', () => {
        currentPanel = vscode.window.createWebviewPanel('flowsight', 'FlowSight Monitor', vscode.ViewColumn.Two, { enableScripts: true, retainContextWhenHidden: true });
        currentPanel.webview.html = getWebviewContent();
        currentPanel.onDidDispose(() => { currentPanel = undefined; }, null, context.subscriptions);
    }));
}

export function deactivate() { if (server) server.close(); }
